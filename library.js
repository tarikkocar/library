...
function displayBooks() {
  for (let i = 0; i < myLibrary.length; i++) {
    if (myLibrary[i].listed === false) {
      ...
      book.innerHTML = `<td>${myLibrary[i].title}</td><td>${
        myLibrary[i].author
      }</td><td>${myLibrary[i].pages}</td><td>${
        myLibrary[i].read ? "Read" : "Not read"
      }</td><td>${
        myLibrary[i].date
      }</td><td class="settings"><div><button class="edit" id="${
        myLibrary[i].id
      }">Edit</button><button class="delete">X</button></div></td>`;
      myLibrary[i].read
...
function updateBook(e) {
  ...
  for (let i = 0; i < myLibrary.length; i++) {
    if (parseFloat(e.target.id) === parseFloat(myLibrary[i].id)) {
      ...
      book.setAttribute(
        "class",
        `${(myLibrary[i].read = read ? "read" : "not-read")}`
      );
    }
  }
...
function deleteBook(e) {
  ...
  if (verifyDeletion) {
    for (let i = 0; i < myLibrary.length; i++) {
      if (
        parseFloat(e.target.parentNode.childNodes[0].id) ===
        parseFloat(myLibrary[i].id)
      ) {
...
let stuart = new Book(
  ...
  false,
  "",
  "title=Shuggie+Bain&author=Douglas+Stuart&pages=536&date="
);
let solstad = new Book(
  "Shyness and Dignity",
  "Dag Solstad",
  "150",
  true,
  "2022-11-03",
  "title=Shyness+and+Dignity&author=Dag+Solstad&pages=150&read=on&date=2022-11-03"
);
let bolano = new Book(
  ...
  true,
  "2021-08-12",
  "title=The+Savage+Detectives&author=Roberto+Bolaño&pages=610&read=on&date=2021-08-12"
);
